...
public class Connector extends LifecycleMBeanBase  {
    ...
    public Connector(String protocol) {
        ...
        //此处的protocol默认是Http1.1和Ajp3.0两个均在serve.xml中配置
    ...
    protected void initInternal() throws LifecycleException {
        ...
        // Initialize adapter
        //创建一个适配器 --- 可以用Connector关联到Engine的适配器
        adapter = new CoyoteAdapter(this);
        //protocolHandler在此处是一个endpoint
        ...
        //这个判断指的是 是apr的模式但是生命周期的监控中却显示不是apr所以要抛出异常
        if (protocolHandler.isAprRequired() && !AprLifecycleListener.isAprAvailable()) {
            throw new LifecycleException(sm.getString("coyoteConnector.protocolHandlerNoApr", getProtocolHandlerClassName()));
        }

        //这个分支是apr模式访问的  且是https安全模式  如果是apr模式是不会走这个吧？？？因为apr的protocol并未继承AbstractHttp11JsseProtocol
        ...
        try {
            ...
            //初始化protocolHandler
            //调用的是org.apache.coyote.AbstractProtocol.init
