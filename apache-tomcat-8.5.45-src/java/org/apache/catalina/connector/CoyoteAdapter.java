...
public class CoyoteAdapter implements Adapter {
    ...
    public void service(org.apache.coyote.Request req, org.apache.coyote.Response res)
        ...
        try {
            ...
            // request parameters  在mapper中解析请求
            ...
            if (!async && postParseSuccess) {
                ...
                //如果上下文为null，则端点可能是关闭，此连接已关闭，并且请求在其中回收一个不同的线程。该线程将更新访问权限
                //日志，这样就可以不更新访问日志了
                //另一种可能性是在早期发生了错误  正在处理，并且请求无法映射到上下文。
                //在这种情况下，通过主机或引擎登录。
     ...
     *                          对请求进行预处理，如对路径去除分号表示的路径参数、进行URI解码、规格化（点号和两点号）
     */
    protected boolean postParseRequest(org.apache.coyote.Request req, Request request, org.apache.coyote.Response res, Response response)
        ...
        // 如果处理器设置了方案（AJP执行此操作，则HTTP执行以下操作 已启用SSL），也可以使用它来设置安全标志。如果
        // 处理器尚未设置，请使用连接器中的设置
        if (req.scheme().isNull()) {
            // Use connector scheme and secure configuration, (defaults to "http" and false respectively)("http")
            //connector.getScheme()获取的是http还是https
            req.scheme().setString(connector.getScheme());
            //如果不是https就把安全设置成法false
        ...
        /**
         * proxyPort的属性就是用来我这种nginx做前端代理服务器Tomcat在后端处理动态请求的情况的。
         * 假如nginx的端口不是80而是1443这时候就要加上配置proxyPort="10443"
         * 因为如果不配置proxyPort默认就是0，不添加端口当然就默认走了80端口
         */
        if (proxyPort != 0) {
        ...
        //MessageBytes中有四种类型T_BYTES是其中一种 因为网络请求是字节的形式所以会走下边的分支
        /**
        ...
        if (undecodedURI.getType() == MessageBytes.T_BYTES) {
            ...
            // 这一步只是将未解码的 URL 字节复制给 decodedURL  bytechunk
        ...
        //是否开启虚拟主机  <host name="www.example.com" appBase="/home/example/webapp">
                    ...
                    for (int i = contexts.length; i > 0; i--) {
                        ...
                        if (ctxt.getManager().findSession(sessionID) != null) {
                            ...
                            if (!ctxt.equals(request.getMappingData().context)) {
                                // Set version so second time through mapping the correct context is found
        ...
        // Filter trace method 判断方法类型是否是 TRACE 类型的方法，如果是，则返回405.不允许该方法进入服务器
