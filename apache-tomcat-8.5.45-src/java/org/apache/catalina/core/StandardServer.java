...
public final class StandardServer extends LifecycleMBeanBase implements Server {
    ...
    protected void startInternal() throws LifecycleException {
        ...
        //先去执行下看看有没有监听configure_start这个状态的
        fireLifecycleEvent(CONFIGURE_START_EVENT, null);
        //再去执行看看有没有监听start这个状态的
        setState(LifecycleState.STARTING);

        //启动了命名资源  为了支持JNDI
        //这个组件的作用就是将配置文件中的不同资源及其属性映射到内存中，这些映射统一由NamingResource对象封装
    ...
    protected void initInternal() throws LifecycleException {
         ...
         * 翻译：
         * 注册全局字符串缓存
         * 注意尽管缓存是全局的，但是如果有多个服务器
         * 存在于JVM中（嵌入时可能会发生），然后存在相同的缓存
         * 注册了全局字符串缓存，注册了MBeanFactory
         *
         */
        ...
        // class loaders(类加载器)
        // 在catalina中执行了这个方法getServer()。setCatalina(this);
        ...
        if (getCatalina() != null) {
            ...
            //遍历类加载器的层次结构。在系统类加载器处停止（jvm中的类加载器）。
        ...
        //开始初始化service  在此处就验证了一个server会有多个service，把所有的service全部启动
        for (int i = 0; i < services.length; i++) {
            services[i].init();
        }
    }
