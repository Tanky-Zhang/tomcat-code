...
public final class ApplicationFilterChain implements FilterChain {
     ...
     * doFilter方法也遵循了jee的规范 实现了统一的filter的接口
     */
    @Override
    ...
    private void internalDoFilter(ServletRequest request,
        ...
        if (pos < n) {
            ...
            try {
                ...
                if( Globals.IS_SECURITY_ENABLED ) {
                    ...
                    //调用过滤链  在这里的处理非常的巧妙  这里是一次递归调用 当filter调用了dochain的方法以后
                    //在应用程序定义的filter中最后一定会执行一句filterchain.dofilter而此时会重新调用本类的dofilter方法
                    //然后这样递归  总有一次会出现pos>=n这时候不会进入这个分支而是直接执行servlet等servlet执行完毕就会就会一次return
                    //设计巧妙！！！！！！！！！！！！牛逼！！！！！！！！！！！！！！！！
        ...
        try {
            ...
            if ((request instanceof HttpServletRequest) && (response instanceof HttpServletResponse) && Globals.IS_SECURITY_ENABLED ) {
                ...
                //安全的调用 service方法通过反射调用的
                SecurityUtil.doAsPrivilege("service", servlet, classTypeUsedInService, args, principal);

            } else {

                //调用了servlet的service方法 这便是容器的开发和应用的开发都要遵循的servlet的规范
                //容器的开发者要调用service方法而应用的开发者要实现service方法
