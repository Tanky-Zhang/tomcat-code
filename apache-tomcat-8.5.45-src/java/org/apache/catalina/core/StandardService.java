...
public class StandardService extends LifecycleMBeanBase implements Service {
    ...
    protected void startInternal() throws LifecycleException {
        ...
        //设置状态 每当设置完状态就要启动下那六个监听器，通过状态来判断他们这时候该做的事情
    ...
    protected void initInternal() throws LifecycleException {
        ...
        // Initialize any Executors  初始化一个线程池
        //StandardThreadExecutor  executor同样继承了lifecyle初始化了线程池
        //可以在server.xml中配置多个线程池，供Connector使用
        ...
        // 会发现如果engine不进行初始化则不会进行他的初始化
        //通过监听容器的AFTER_START_EVENT事件来对容器进行注册；
        //通过监听容器的BEFORE_STOP_EVENT事件来完成对容器的取消注册。
        //这一步总体而言只是将自己注册进了JMX  这是一个负责映射的监听器 路由监听
        mapperListener.init();

        // Initialize our defined Connectors
        //一个service下会有多个connector  所以要保证不会出现线程安全问题
        synchronized (connectorsLock) {
            for (Connector connector : connectors) {
                try {

                    //connector的初始化 connector--初始化---->protocol--初始化-->endpoint
                    connector.init();
