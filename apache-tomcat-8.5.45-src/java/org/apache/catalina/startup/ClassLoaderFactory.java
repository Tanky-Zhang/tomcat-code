...
public final class ClassLoaderFactory {
     ...
     * @exception Exception if an error occurs constructing the class loader
     *
     * 类加载器的工厂顾名思义：生产一个类加载器，其类型为URLClassloader,在java中AppClassloader和
     * ExtClassloader全都继承于URLClassloader,而URLClassloader继承自Classloader和实现了Closeable接口
     * 再此处如果是commonClassloader传入则会将他的父类加载器设为AppClassloader,此处的体现在Classloader类中
     * 的无参构造方法 this(checkCreateClassLoader(), getSystemClassLoader()); 此时应该注意的是commonClassloader
     * 并没有重写loadClass()方法，也就意味着此处没有通过重写方法打破双亲委派机制
     *
     */
