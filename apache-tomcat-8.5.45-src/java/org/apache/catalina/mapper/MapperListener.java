...
public class MapperListener extends LifecycleMBeanBase  implements ContainerListener, LifecycleListener {
     ...
     *
     *  将mapper和service存储到当前对象的相关属性中。
     *                mapper是在service下的他保存了映射关系
     */
    ...
    private void prepareWrapperMappingInfo(Context context, Wrapper wrapper, List<WrapperMappingInfo> wrappers) {
        //获取应用的名字  例如：/manager  /host-manager  /docs
        String wrapperName = wrapper.getName();
        ...
        for (String mapping : mappings) {
            ...
            //封装一个wrapper信息包括路径包括映射的路径 wrapperName是否是jsp  或者以/* 结尾
    ...
    private void addListeners(Container container) {
        //把自己作为监听器注册进engine，从此多了一个监听器
        container.addContainerListener(this);
        container.addLifecycleListener(this);
        for (Container child : container.findChildren()) {
            //递归调用将所有的子组件都注册上这个监听器是所有  包括host的子组件  即所有的容器
            addListeners(child);
        }
    }
